...
def init_state(size):
    state = np.zeros((draughts_vars.NUM_CHANNELS, size, size))
    return state

def batch_init_state(batch_size, board_size):
    batch_state = np.zeros(batch_size, draughts_vars.NUM_CHANNELS, board_size, board_size)
    return batch_state
...
def next_state(state, action1d, canonical=False):
    ...
    if passed:
        # We passed
        state[draughts_vars.PASS_CHANNEL] = 1
        if previously_passed:
            # Game ended
            state[draughts_vars.DONE_CHANNEL] = 1
    else:
        # Move was not pass
        state[draughts_vars.PASS_CHANNEL] = 0

        # Assert move is valid
        assert state[draughts_vars.INVD_CHANNEL, action2d[0], action2d[1]] == 0, ("Invalid move", action2d)
    ...
    state[draughts_vars.INVD_CHANNEL] = state_utils.compute_invalid_moves(state, player, ko_protect)
...
def batch_next_states(batch_states, batch_action1d, canonical=False):
    ...
    # Pass moves
    batch_states[batch_pass, draughts_vars.PASS_CHANNEL] = 1
    # Game ended
    batch_states[batch_game_ended, draughts_vars.DONE_CHANNEL] = 1

    # Non-pass moves
    batch_states[batch_non_pass, draughts_vars.PASS_CHANNEL] = 0

    # Assert all non-pass moves are valid
    assert (batch_states[batch_non_pass, draughts_vars.INVD_CHANNEL, batch_action2d[:, 0], batch_action2d[:, 1]] == 0).all()
...
def invalid_moves(state):
    ...
    return np.append(state[draughts_vars.INVD_CHANNEL].flatten(), 0)
...
def batch_invalid_moves(batch_state):
    n = len(batch_state)
    batch_invalid_moves_bool = batch_state[:, draughts_vars.INVD_CHANNEL].reshape(n, -1)
...
def prev_player_passed(state):
    return np.max(state[draughts_vars.PASS_CHANNEL] == 1) == 1


def batch_prev_player_passed(batch_state):
    return np.max(batch_state[:, draughts_vars.PASS_CHANNEL], axis=(1, 2)) == 1
...
def game_ended(state):
    ...
    """
    m, n = state.shape[1:]
    return int(np.count_nonzero(state[draughts_vars.DONE_CHANNEL] == 1) == m * n)
...
def batch_game_ended(batch_state):
    ...
    :return: 0/1 = game not ended / game ended respectively
    """
    return np.max(batch_state[:, draughts_vars.DONE_CHANNEL], axis=(1, 2))
...
def turn(state):
    """
    :param state:
    :return: Who's turn it is (draughts_vars.BLACK/draughts_vars.WHITE)
    """
    return int(np.max(state[draughts_vars.TURN_CHANNEL]))


def batch_turn(batch_state):
    return np.max(batch_state[:, draughts_vars.TURN_CHANNEL], axis=(1, 2)).astype(np.int)


def liberties(state: np.ndarray):
    blacks = state[draughts_vars.BLACK]
    whites = state[draughts_vars.WHITE]
    all_pieces = np.sum(state[[draughts_vars.BLACK, draughts_vars.WHITE]], axis=0)
...
def areas(state):
    ...
    black_area, white_area = np.sum(state[draughts_vars.BLACK]), np.sum(state[draughts_vars.WHITE])
    for label in range(1, num_empty_areas + 1):
        empty_area = empty_labels == label
        neighbors = ndimage.binary_dilation(empty_area)
        black_claim = False
        white_claim = False
        if (state[draughts_vars.BLACK] * neighbors > 0).any():
            black_claim = True
        if (state[draughts_vars.WHITE] * neighbors > 0).any():
            white_claim = True
...
def canonical_form(state):
    state = np.copy(state)
    if turn(state) == draughts_vars.WHITE:
        channels = np.arange(draughts_vars.NUM_CHANNELS)
        channels[draughts_vars.BLACK] = draughts_vars.WHITE
        channels[draughts_vars.WHITE] = draughts_vars.BLACK
...
def batch_canonical_form(batch_state):
    ...
    white_players_idcs = np.nonzero(batch_player == draughts_vars.WHITE)[0]
    ...
    for i in white_players_idcs:
        ...
        batch_state[i, draughts_vars.TURN_CHANNEL] = 1 - batch_player[i]
...
def random_action(state):
    """
    Assumed to be (NUM_CHANNELS, BOARD_SIZE, BOARD_SIZE)
    Action is 1D
    """
    invalid_moves = state[draughts_vars.INVD_CHANNEL].flatten()
